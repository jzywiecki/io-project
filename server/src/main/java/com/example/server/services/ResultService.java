...
public class ResultService {
     ...
     */
    public ResultsDto getResults(long roomId) {
     ...
     */
    public UserResultsDto getUserResults(long roomId, long userId) {
        checkDateDeadline(roomId);
        ...
        Result result = resultRepository.findByRoomIdAndUserId(roomId, userId)
    ...
    private void checkDateDeadline(long roomId){
        Room room = roomRepository.findById(roomId).orElseThrow(() -> new RoomNotFoundException("Room not found."));

        boolean isAfterToday = room.getDeadlineDate().toLocalDate().isAfter(LocalDateTime.now().toLocalDate());

        if (!isAfterToday) {
            boolean isToday = room.getDeadlineDate().toLocalDate().isEqual(LocalDateTime.now().toLocalDate());
            if (isToday) {
                boolean isAfterNow = room.getDeadLineTime().toLocalTime().isAfter(LocalDateTime.now().toLocalTime());
                if (isAfterNow) {
                    throw new DeadlineHasNotPassedException("The deadline has not passed yet.");
                }
            }
        }
    }
